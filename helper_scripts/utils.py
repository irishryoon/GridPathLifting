# This is a copy of utils.py from Gardner et al repository
# https://github.com/erikher/GridCellTorus/blob/main/utils.py
...
import time
import os
from constants import GARDNER_CONJUNCTIVE_PATH
...
def pca(data, dim=2):
    ...
    components = np.real_if_close(components)
...
def get_coords(cocycle, threshold, num_sampled, dists, coeff):
    ...
    for i in range(num_edges):
        # print(v1.shape, np.where(verts == edges[0][i])[0])
        v1[i, :] = np.array([i, np.where(verts == edges[0][i])[0][0]])
        v2[i, :] = np.array([i, np.where(verts == edges[1][i])[0][0]])
...
def get_spikes(rat_name, mod_name, day_name = '', sess_name = '', bType = None, bSmooth = True, 
    ...
    if str.upper(rat_name) == 'R':
        f = np.load(os.path.join(folder, f'rat_r_{day_name}_grid_modules_1_2_3.npz'), allow_pickle = True)
    elif str.upper(rat_name) == 'Q':
        f = np.load(os.path.join(folder, 'rat_q_grid_modules_1_2.npz'), allow_pickle = True)
    elif str.upper(rat_name) == 'S':
        f = np.load(os.path.join(folder, 'rat_s_grid_modules_1.npz'), allow_pickle = True)
    else:
    ...
    if bType:    
        f = np.load(GARDNER_CONJUNCTIVE_PATH, allow_pickle = True)
        if len(day_name)>0:
